...
|---------|---------|-------|-------------|-------|------------|
| v38 | — | 2026-07-11 | Anti-ráfaga tras test de Kenny (12:21): DeepL free throttlea POR IP (las 3 cuentas dieron 429 a la vez) y la cascada lo alimentaba (4x peticiones). Fix: breaker 10s por key tras 429 (salta, no espera); captions parciales con UN intento (key primaria, sin cascada ni Gemini) y traducción máx cada 1.5s (menos rate y ~mitad de chars). El 2º fallo del test (12:23) fue el internet de Kenny (DNS caído ~10s), no la app. | ⏳ | Solo backend; reiniciada por Claude. Las filas ⚠ del test se recuperan con clic. |
